...
class Index_articles_admin extends CI_Controller
{
...
////////////////////////////////ARTICLES//////////////////////////
    ...
    public function ajax_send_article_to_subscribers($articleId)
    {
        ...
        try{
            ...
            $articleDetail  = $articlesArr ? $articlesArr[0] : null;
            Common::assertTrue($articleDetail, 'Ошибка! Нет данных по запрашиваемой статье');
            
            $recipientsArr = $this->index_model->getNlSubscribers();
            Common::assertTrue($recipientsArr, 'Не найден ни один подписчик для отправки');
            
            $sentMailCounter = $this->_sendNlSubscribe($recipientsArr, $articleDetail);
//            Common::assertTrue($sentMailCounter < 1, 'Ошибка! Не было отправлено ни одного письма');
            
            $isUpdated = $this->_updateArticleStatusIsMailSent($articleDetail['id']);
            Common::assertTrue($isUpdated, 'Ошибка! Статус сатьи не был изменен на articles');
            
            $this->result['success'] = 'Рассылка прошла успешно! <br>Можно идти пить чай:)';
