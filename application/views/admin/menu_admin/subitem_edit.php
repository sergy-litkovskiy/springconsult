...
<div class="edit_form">
    <form action='<?php echo base_url()?>backend/check_valid_submenu' method='post' name='edit_text' enctype='multipart/form-data'>
        <p><b>Раздел для подраздела:</b></p>
        <select id='parent' name='parent'>
            <option value="">--Выбрать раздел--</option>
            <?php foreach($itemArr as $item):?>
                <option value="<?php echo $item['id']?>" <?php if ($content['parent'] == $item['id']) echo "selected = selected";?>><?php echo $item['title']?></option>
            <?php endforeach;?>
        </select>
        ...
        <!--        <input type="text" id='slug' name='meta_description' value="<?php echo $content['meta_description'];?>"/>-->
        <div style="width:600px; clear:both">&nbsp;</div>
        <p><b>Meta keywords:</b></p>
        <input type="text" id='slug' name='meta_keywords' value="<?php echo $content['meta_keywords'];?>"/>
